...
export default function ThemeSwitcher({ mobile }: Props) {
  const [currentTheme, setTheme] = useState<string | undefined>(undefined);
  ...
  useEffect(() => {
    if (currentTheme) {
      ...
      document.cookie = `theme=${currentTheme};expires=` + new Date(new Date().getTime() + 400 * 24 * 60 * 60 * 1000).toUTCString();
        ...
        <>
          {mobile ? (
            ...
            <label className="swap swap-rotate w-11 h-auto">
              <input
                aria-label={currentTheme === "dark" ? "Включить светлую тему" : "Включить темную тему"}
              ...
              <svg
                ...
                viewBox="0 0 24 24"
                area-hidden={(currentTheme === "dark").toString()}
              >
              ...
              <svg
                ...
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                area-hidden={(currentTheme === "lighs").toString()}
              >
